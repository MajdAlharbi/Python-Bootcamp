...
app_count = input("How many apps do you want to create? ").strip()

if not app_count.isdigit() or int(app_count) <= 0:
    print("Error: App count must be a positive number.")
    sys.exit()

app_names = []
for app_number in range(1, int(app_count) + 1):
    app_name = input(f"Enter app #{app_number} name: ").strip()
...
if not project_name or any(not app_name for app_name in app_names):
