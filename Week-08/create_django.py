...
for raw in input("Enter app names, comma separated (e.g. home,accounts): ").split(","):
    ...
    if not VALID_NAME.match(name):
        fail(
            f"'{name}' is not a valid app name. Use lowercase letters, "
...
for app in app_names:
    write(os.path.join(project_path, app, "urls.py"), APP_URLS.replace("__APP__", app))

    write(
        os.path.join(project_path, app, "views.py"), APP_VIEWS.replace("__APP__", app)
    )
    ...
    os.makedirs(os.path.join(project_path, app, "templates", app), exist_ok=True)
